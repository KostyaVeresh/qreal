#include "gestRectangle.h"

using namespace GestEditor;

GestRectangle::GestRectangle(qreal x1, qreal y1, qreal x2, qreal y2, QGraphicsItem *parent) : GestItem(x1, y1, x2, y2)
{
	mItemType = "rectangle";
}

void GestRectangle::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
	if (this->isSelected()) {
		drawRect(painter);
		drawIdeal(painter);
...
void GestRectangle::drawItem(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
	painter->drawRect(rectForDraw());
}

bool GestRectangle::isIdeal() const
{
	return (rectForDraw().width() == rectForDraw().height());
}

void GestRectangle::drawIdeal(QPainter *painter)
{
...
void GestRectangle::transformToIdeal()
{
	int size = std::min(rectForDraw().width(), rectForDraw().height());
	int autoChange = size / partOfIdeal;
	if (sizePolicy() < autoChange) {
...
void GestRectangle::drawScalingRects(QPainter *painter) {
...
void GestRectangle::changeDragState(qreal x, qreal y) {
...
void GestRectangle::updateScalingRects(QPointF topLeft, QPointF bottomRight) {
	qreal X1 = topLeft.x();
	qreal Y1 = topLeft.y();
...
void GestRectangle::updatePorts()
{
	mDockingRects.clear();
...
void GestRectangle::connectPoints(QPointF oldPoint, QPointF newPoint)
{
...
void GestRectangle::findPortRectsAndPoints()
{
...
QDomElement GestRectangle::generateItem(QDomDocument &document)
{
...
QList<QPoint> GestRectangle::getCurve(QPoint topLeftPoint)
{
