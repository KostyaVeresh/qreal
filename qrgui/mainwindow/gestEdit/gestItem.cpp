...
GestItem::GestItem(qreal x1, qreal y1, qreal x2, qreal y2, QGraphicsItem *parent) :
...
QRectF GestItem::boundingRect() const
{
	QPoint topLeft;
	QPoint bottomRight;
...
QRectF GestItem::rectForDraw() const
{
...
void GestItem::setX1andY1(qreal x, qreal y)
{
	mX1 = x;
	mY1 = y;
}

void GestItem::setX2andY2(qreal x, qreal y)
{
	mX2 = x;
	mY2 = y;
}

void GestItem::setX1andY2(qreal x, qreal y)
{
	mX1 = x;
	mY2 = y;
}

void GestItem::setX2andY1(qreal x, qreal y)
{
	mX2 = x;
	mY1 = y;
}

void GestItem::paint(QPainter* painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
	painter->setPen(mPen);
...
void GestItem::drawRect(QPainter *painter) const
{
...
void GestItem::drawPorts(QPainter *painter)
{
...
void GestItem::resizeItem(QGraphicsSceneMouseEvent *event) {
	if (mDragState != None) {
...
void GestItem::calcResizeItem(QGraphicsSceneMouseEvent *event) {
...
QPointF GestItem::getX1Y1() const
{
	return QPointF(mX1, mY1);
}

QPointF GestItem::getX2Y2() const
{
	return QPointF(mX2, mY2);
}

QPointF GestItem::getX1Y2() const
{
	return QPointF(mX1, mY2);
}

QPointF GestItem::getX2Y1() const
{
	return QPointF(mX2, mY1);
}

void GestItem::setOrientation() {
	if (mX1 <= mX2) {
		if (mY1 <= mY2) {
...
void GestItem::intersectPorts(GestItem *connectingItem)
{
...
void GestItem::findDocPoint(GestItem *connectingItem)
{
...
bool GestItem::closeEnough(QPointF firstPoint, QPointF secondPoint) const
{
...
int GestItem::countRatio(int part) const
{
...
void GestItem::insertConnectedPorts(QVector<QPointF> points)
{
	mConnectedPorts.clear();
	mConnectedPorts = points;
}

QDomElement GestItem::generateItem(QDomDocument &document)
{
	QDomElement dom = document.createElement("Not stated!");
	return dom;
}

void GestItem::setXandY(QDomElement& dom)
{
...
QString GestItem::getName() const
{
	return mItemType;
}
