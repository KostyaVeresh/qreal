...
GestCurve::GestCurve(qreal x1, qreal y1, qreal x2, qreal y2, QGraphicsItem *parent)
...
QRectF GestCurve::helpRectForDraw()
{
	int width = 0;
	int height = 0;
	QPoint topLeft;
...
void GestCurve::drawItem(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
	setOrientation();
...
void GestCurve::setSpanAngle(int spanAngle)
{
	mSpanAngle = spanAngle;
}

void GestCurve::setStartAngle(int startAngle)
{
	mStartAngle = startAngle;
}

bool GestCurve::isIdeal() const
{
	return (rectForDraw().width() == rectForDraw().height());
}

void GestCurve::drawIdeal(QPainter *painter)
{
...
void GestCurve::transformToIdeal()
{
	int size = (boundingRect().width() + boundingRect().height()- 4 * scalingDrift) / 2;
...
void GestCurve::drawScalingRects(QPainter *painter)
{
	painter->setPen(mPen);
...
void GestCurve::updateScalingRects(QPointF topLeft, QPointF bottomRight)
{
	qreal X1 = topLeft.x();
	qreal Y1 = topLeft.y();
...
void GestCurve::resizeItem(QGraphicsSceneMouseEvent *event)
{
...
void GestCurve::changeDragState(qreal x, qreal y)
{
...
void GestCurve::connectPoints(QPointF oldPoint, QPointF newPoint)
{
...
QDomElement GestCurve::generateItem(QDomDocument &document)
{
...
void GestCurve::setXandY(QDomElement& dom)
{
...
QList<QPoint> GestCurve::getCurve(QPoint topLeftPoint)
{
