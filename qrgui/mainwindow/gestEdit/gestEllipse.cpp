...
GestEllipse::GestEllipse(qreal x1, qreal y1, qreal x2, qreal y2, QGraphicsItem *parent)
	: GestItem(x1, y1, x2, y2)
{
	mItemType = "ellipse";
}

void GestEllipse::drawItem(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
	painter->setPen(mPen);
	painter->drawEllipse(rectForDraw());
}

bool GestEllipse::isIdeal() const
{
	return (rectForDraw().width() == rectForDraw().height());
}

void GestEllipse::drawIdeal(QPainter *painter)
{
...
void GestEllipse::transformToIdeal()
{
...
void GestEllipse::drawScalingRects(QPainter *painter)
{
	painter->setPen(mPen);
...
void GestEllipse::changeDragState(qreal x, qreal y)
{
...
void GestEllipse::updateScalingRects(QPointF topLeft, QPointF bottomRight)
{
	qreal X1 = topLeft.x();
	qreal Y1 = topLeft.y();
...
void GestEllipse::connectPoints(QPointF oldPoint, QPointF newPoint)
{
...
void GestEllipse::findPortRectsAndPoints()
{
...
QDomElement GestEllipse::generateItem(QDomDocument &document)
{
...
QList<QPoint> GestEllipse::getCurve(QPoint topLeftPoint)
{
